...
The service detects the Cloud Storage mount automatically. It coordinates leases and snapshot fencing through conditional object writes. Multiple Cloud Run instances are safe. Local development uses an exclusive file lease.
...
Cloud Run requires a Cloud Storage volume at `EXPERIMENTS_DIR`. The service refuses lease work when Cloud Run is detected without that mount. Application Default Credentials and the service identity's bucket role authorize conditional object operations. No key file is needed.
