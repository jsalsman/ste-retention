...
they never need a live key or paid call. As deployment checklist gates, the image
build also parses root `index.html` with Jinja's parser and briefly starts the
production `gthread` command to check `/healthz` and the expected root content.
These checks remain in the Dockerfile rather than in pytest or a separate
container-build test suite. If Hadolint is already installed, `hadolint
Dockerfile` is an optional lightweight syntax/style check.
