...
The current page is a table. It shows the model, protocol version, scoring version, bare baseline, three mean contrasts, paired-observation count, and a two-sided 95% confidence interval for every estimate. Each effect is calculated inside a complete matched four-arm cell before cells are averaged. The baseline interval likewise uses only the matched bare scores. Intervals use the mean plus or minus 1.96 standard errors; with fewer than two cells, the point estimate remains visible and the interval is marked unavailable.

The interval calculation treats complete cells as independent. It is not a clustered or hierarchical analysis, so pooled cells from different sessions, depths, or runs may contain variation at more than one level. An interval describes uncertainty in the aggregate estimate; it is not the range of individual scores or a 95% probability statement about the fixed population effect. Wider intervals represent greater uncertainty and should not be reduced to a binary significance label.
