"""Detect local or Cloud Storage persistence and verify mount-to-object mapping."""
...
class StorageBackend:
    """Describe either local storage or a verified Cloud Storage bucket mapping."""

    directory: Path
    mount: GCSMount | None = None
    bucket: Any | None = None

    @property
    def is_gcs(self) -> bool:
        """Return whether coordination must use Cloud Storage object operations."""
        # Callers use this branch before attempting any filesystem lock or rename.
    ...
    def object_name(self, relative_name: str) -> str:
        ...
        # Local callers never need an object name, so reject accidental misuse.
        if self.mount is None:
            raise LeaseUnavailableError("Cloud Storage coordination is unavailable.")
...
def _new_client() -> Any:
    """Create an ADC-backed client lazily so local execution needs no GCS setup."""
...
def get_backend(
    ...
    with _BACKENDS_LOCK:
        ...
        if mount is None:
            ...
            backend = StorageBackend(resolved)
        else:
            try:
                client = _new_client()
