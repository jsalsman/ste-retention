...
def chat(
    ...
    *,
    ...
    # Track the latest validated truncation status for any later continuation failure.
    last_truncation_reason = "length"
    ...
    try:
        ...
        for attempt in range(MAX_CONTINUATIONS + 1):
            ...
            # Expiry is an incomplete result only after useful provider work was validated.
            if remaining <= 0:
                if content_parts:
                    # Preserve paid partial work instead of making a runner repeat the call.
                    raise IncompleteGenerationError("".join(content_parts), last_truncation_reason)
                # Without any model text, callers must treat expiry as a failed inference.
                raise UpstreamError("OpenRouter could not complete the request.")
            ...
            # Remember which supported token-limit status prompted this continuation.
            last_truncation_reason = normalized_reason
        ...
        # A later failed call must not discard chunks validated on earlier paid calls.
        if content_parts:
            # Suppress unsafe provider details while exposing only accumulated model text.
            raise IncompleteGenerationError(
                "".join(content_parts), last_truncation_reason
            ) from None
