...
def test_backend_is_local_outside_cloud_run(tmp_path, monkeypatch):
    """Select local locking when no gcsfuse mount or Cloud Run service exists."""
    monkeypatch.delenv("K_SERVICE", raising=False)
    mountinfo = _mountinfo(tmp_path, "1 2 3 4 / rw - ext4 disk rw\n")
    # A unique directory avoids interaction with process-level detection caching.
    backend = get_backend(tmp_path / "local", mountinfo)
    assert not backend.is_gcs


def test_backend_refuses_cloud_run_without_gcsfuse(tmp_path, monkeypatch):
    """Reject unsafe process-local coordination in a misconfigured Cloud Run service."""
